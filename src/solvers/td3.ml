  ...
  struct
    ...
    let solve box st vs data =
      ...
      if !incremental_mode = "incremental" then (
        ...
        let part_changed_funs = filter_map (fun c -> match c.old, c.diff with GFun (f,l), Some d -> Some (f,d) | _ -> None) S.increment.changes.changed in
        ...
        *)
        ...
        List.iter (fun (f,d) -> let pid = f.svar.vid + 10_000_000_000 in if not (List.exists (fun (n1,_) -> match n1 with MyCFG.Statement s -> s.sid = pid | _ -> false) d.unchangedNodes) then Hashtbl.replace marked_for_deletion (string_of_int pid) ()) part_changed_funs;
