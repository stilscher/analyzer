...
df.rename(columns={'l_max': 'changed LOC', 'time-orig-avg': 'Run time of the basic incremental analysis', 'time-new-avg': 'Run time of the analysis run with intra-functional changes'}, inplace=True)

df.plot(x="index_commit", y=["changed LOC", "Run time of the basic incremental analysis", "Run time of the analysis run with intra-functional changes"], kind="bar", yerr={'Run time of the basic incremental analysis': df['time-orig-var'], 'Run time of the analysis run with intra-functional changes': df['time-new-var']}, width=0.7, figsize=(8,6), capsize=2)
plt.ylim(top=20)
plt.xlabel("Commit")
