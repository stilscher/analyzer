...
df.rename(columns={'l_max': 'changed LOC', 'time-orig-avg': 'Runtime of the basic incremental analysis in s', 'time-new-avg': 'Runtime of the analysis run with intra-functional changes in s'}, inplace=True)

df.plot(x="index_commit", y=["changed LOC", "Runtime of the basic incremental analysis in s", "Runtime of the analysis run with intra-functional changes in s"], kind="bar", yerr={'Runtime of the basic incremental analysis in s': df['time-orig-var'], 'Runtime of the analysis run with intra-functional changes in s': df['time-new-var']}, width=0.7, figsize=(8,6), capsize=2)
plt.ylim(top=20)
plt.xlabel("Commit")
plt.legend(loc="upper right")
plt.savefig("avg_time_figlet_intra_func_changes.pdf", bbox_inches = 'tight', pad_inches = 0)
